...
class AutomaticUpgrader:
    ...
    def save_last_run(self):
        ...
        with open(os.path.join(sys_path.pc_cache_dir(), "last_run.json"), "w") as fobj:
            fobj.write(json.dumps({"timestamp": int(time.time()), "st_version": self.current_version}))
    ...
    async def upgrade_packages(self):
        ...
        with ActivityIndicator("Searching updates...") as progress:
            ...
            missing_libraries = await upgrader.manager.find_missing_libraries(
                required_libraries=required_libraries
            )
            await upgrader.manager.install_libraries(
                libraries=required_libraries - missing_libraries, fail_early=False
            )
